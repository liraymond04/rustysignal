    ...
    fn handle_push_requests(&mut self, json_message: &Value) {
        match json_message["action"].as_str() {
            ...
            Some("connection-request") => match json_message["endpoint"].as_str() {
                ...
                _ => {
                    println!("No endpoint for connection request")
                }
            },
...
impl Handler for Server {
    fn on_open(&mut self, handshake: Handshake) -> Result<()> {
        ...
        // i.e localhost:8000/?user=testuser&room=roomcode

        // Skip everything before the first '?' to allow the server to run behind a reverse proxy like nginx with minimal fuss
        let url_arguments = handshake
            .request
            .resource()
            .split(|c| c == '?' || c == '=' || c == '&')
            .skip(1);
        // Collecting pairs into a vector
        let argument_vector: Vec<&str> = url_arguments.collect();

        // Create a HashMap to store key-value pairs
        let mut arguments = std::collections::HashMap::new();

        // Iterate over the argument_vector and populate the HashMap
        let mut key = "";
        for (i, arg) in argument_vector.iter().enumerate() {
            if i % 2 == 0 {
                key = arg;
            } else {
                arguments.insert(key, *arg);
            }
        }
        ...
        if let Some(&username) = arguments.get("user") {
            if let Some(&room) = arguments.get("room") {
                ...
                return Err(ws::Error {
                    ...
                    details: "New node didn't provide a room code".into(),
                });
            }
        } else {
            return Err(ws::Error {
                kind: ws::ErrorKind::Protocol,
                details: "New node didn't provide a username".into(),
            });
        }
    ...
    fn on_message(&mut self, msg: Message) -> Result<()> {
        ...
        let ret = match protocol {
            ...
            _ => self.node.borrow().sender.send(
